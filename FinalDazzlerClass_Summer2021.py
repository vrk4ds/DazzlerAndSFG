...
import pypret
from CompletetheMeshDataObject import complete_object 
...
class Dazzler_Pulse_Shaper():  
    ...
    def calculate_full_transfer_function(self,ang_freq_vector, a_saved=0, phi_saved=0, components_return=False):
        ...
        #calculate S
        S_full = A_dial*np.exp(1j*phi_dial) + a_saved*np.exp(1j*np.pi*phi_saved)*S_saved
    ...
    def shape_input_pulse_low_sampling(self, E_field_input, time_vector, phi_saved=0, a_saved=0):
        ...
        S_full=self.calculate_full_transfer_function(2*np.pi*freq_vector, phi_saved=phi_saved, a_saved=a_saved)
    ...
    def shape_input_pulse(self, E_field_input, time_vector, wavelength_vec_limits=None,
        ...
        if (components_return):
            f,g,A_dial,phi_dial, S_full = self.calculate_full_transfer_function(ang_freq_vector, phi_saved=phi_saved, a_saved=a_saved, components_return=True)
            components_dict = {"f":f,"g":g,"A_dial":A_dial,"phi_dial":phi_dial,"S_full":S_full}

        else :
            S_full = self.calculate_full_transfer_function(ang_freq_vector, phi_saved=phi_saved, a_saved=a_saved, components_return=False)
        ...
        if (wavelength_vec_limits[0]<self.c/max(np.abs(freq_vector))) or (wavelength_vec_limits[1]>self.c/freq_vector[1]):
            print("Your sampling rate is not sufficient for accurate information in the frequency domain. We will recreate the input field in the frequency domain to avoid the first Fourier transform. The frequency vector will not take the typical form; some fftshifts may no longer be needed. WARNING: this output is still inaccurate if your sampling rate is less than 195/pulsewidth.")
            self.shape_input_pulse_low_sampling(E_field_input, time_vector, phi_saved=phi_saved, a_saved=a_saved)
    ...
    def make_plots(self, functions, domains, E_field_input, E_field_input_ft, E_field_output, E_field_output_ft,
        ...
        output_field_interp = interp1d(2*np.pi*freq_vector, E_field_output_ft)
        ...
        def plot_spectrum_phase(efield,x_vals, title, x_label,x_lims=None):
        ...
        for word1 in functions:
            for word2 in domains:
                plot_spectrum_phase(function_to_fieldset[word1][domain_in_fieldset[word2]],domain_to_xval[word2], word1, word2, x_lims=None)
    ...
    def FROG_retrieval(self, intensityarray, time_vector, freq_vector, guess=np.array([None]), E_field_output_ft=np.array([None]),x_lim=None):
        ...
        MeshDataObject=pypret.MeshData(np.transpose(intensityarray), time_vector, freq_vector)
        pypret.MeshData.axes=time_vector, freq_vector
        
        if guess.any()==None:
            time_vector_irrelevant, guess = self.make_gaussian_pulse(time_vector=time_vector)
        
        ourspec=complete_object(MeshDataObject, time_vector, freq_vector)
        ...
        if E_field_output_ft.any()!=None:
            if x_lim!=None:
                plt.xlim(x_lim[0],x_lim[1])
            norm1=np.amax(np.abs(E_field_retrieved_freq)**2)
            plt.plot(np.fft.fftshift(freq_vector), (np.abs(E_field_retrieved_freq)**2)/norm1)
            norm2=np.amax(np.abs(E_field_output_ft)**2)
            plt.plot(np.fft.fftshift(freq_vector), np.fft.fftshift((np.abs(E_field_output_ft)**2)/norm2))
            #blue is retrieved from trace, orange is output of Dazzler simulation
